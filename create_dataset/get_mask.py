...
import shutil
# 数据集路径
dataset_path = r"semanticSegmentation\DATASET_ALL\cityscapes\gtFine_trainvaltest\gtFine"
new_dataset_path = r"test\dataset"
#原始的train, valid文件夹路径
train_dataset_path = os.path.join(dataset_path,'train')
val_dataset_path  = os.path.join(dataset_path,'val')
test_dataset_path  = os.path.join(dataset_path,'test')
#创建train,valid的文件夹
train_images_path = os.path.join(new_dataset_path,'cityscapes_19classes_train')
val_images_path  = os.path.join(new_dataset_path,'cityscapes_19classes_val')
test_images_path  = os.path.join(new_dataset_path,'cityscapes_19classes_test')
...
for file_name in os.listdir(train_dataset_path):
    ...
    for image in os.listdir(file_path):
        #查找对应的后缀名，然后保存到文件中
        if image.split('.png')[0][-8:] == "labelIds":
            #print(image)
